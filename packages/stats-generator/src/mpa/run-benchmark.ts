...
import type { MPABenchmarkResult, MPARunResult } from './types.ts'
...
async function runOnce(
  ...
  try {
    ...
    // FP + FCP: navigate to /mpa, wait for the table to render
    await flow.navigate(`${url}${MPA_PATH}`)
...
export async function runBenchmark(
  ...
  return {
    ...
    displayName,
    package: packageName,
    mpaFirstPaintMs: avg(fp),
    mpaFCPMs: avg(fcp),
    mpaINPMs: avg(inp),
    mpaRuns: results.length,
  }
}
